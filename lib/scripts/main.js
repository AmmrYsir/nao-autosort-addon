...
const storageSortTokens = new Map();
const storageSortInProgress = new Set();
...
function queueStorageSort(playerId) {
    var _a;
    if (storageSortInProgress.has(playerId))
        return;
    const token = ((_a = storageSortTokens.get(playerId)) !== null && _a !== void 0 ? _a : 0) + 1;
    storageSortTokens.set(playerId, token);
    system.runTimeout(() => {
        if (storageSortTokens.get(playerId) !== token)
            return;
